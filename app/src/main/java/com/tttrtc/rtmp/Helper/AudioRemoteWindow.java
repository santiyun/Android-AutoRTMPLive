...
public class AudioRemoteWindow extends RelativeLayout {
    ...
    public void show(long localId,long id) {
        show(localId, id, 0);
    }
    ...
    public void show(long localId, long id, int oritation) {
        ...
        int i = -1;
        if (localId == id) {
            i = mTTTEngine.setupLocalVideo(new VideoCanvas(0, Constants.RENDER_MODE_HIDDEN, mSurfaceView), oritation);
        } else {
            i = mTTTEngine.setupRemoteVideo(new VideoCanvas(mId, Constants.RENDER_MODE_HIDDEN, mSurfaceView));
        }
        Log.e("###"," window  show setupRemoteVideo = "+i);
